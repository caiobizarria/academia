...
<body class="min-h-screen pb-24 text-slate-100 antialiased selection:bg-cyan-500 selection:text-white">
  ...
  <script>
    ...
    function generateFallbackSVG(title, accent) {
      ...
      const svg = `
        <svg xmlns="http://www.w3.org/2000/svg" width="600" height="400" viewBox="0 0 600 400" fill="none">
          ...
          <circle cx="300" cy="170" r="60" fill="${color}" fill-opacity="0.15" stroke="${color}" stroke-width="3"/>
          <path d="M260 270 L340 270 M300 240 L300 300 M275 160 L325 160 M265 180 L335 180" stroke="${color}" stroke-width="4" stroke-linecap="round"/>
          <text x="300" y="340" text-anchor="middle" fill="#f8fafc" font-family="sans-serif" font-size="20" font-weight="bold">${title}</text>
          <text x="300" y="370" text-anchor="middle" fill="#94a3b8" font-family="sans-serif" font-size="14">Guia Anatômico FitPulse</text>
        </svg>
      `;
      return 'data:image/svg+xml;charset=utf-8,' + encodeURIComponent(svg);
    }

    // High quality, direct gym photo references with instant guaranteed fallbacks
    const exercisesDB = {
      // PUSH
      supino_reto: {
        id: "supino_reto",
      ...
      triceps_corda: {
        ...
        reps: "10 a 15 reps",
        rest: "60s",
        img: "https://images.unsplash.com/photo-1530822847156-5df684ec5ee1?auto=format&fit=crop&w=800&q=80",
      ...
      rosca_direta: {
        ...
        reps: "8 a 12 reps",
        rest: "60s",
        img: "https://images.unsplash.com/photo-1581009146147-975971a93809?auto=format&fit=crop&w=800&q=80",
      ...
      cadeira_extensora: {
        ...
        rest: "60s",
        img: "https://images.unsplash.com/photo-1581009146145-b5ef050c2e1e?auto=format&fit=crop&w=800&q=80",
        muscles: ["Quadríceps Femoral (Reto Femoral, Vasto Lateral, Medial e Intermédio)"],
        steps: [
    ...
    const workoutRoutines = {
      push: ["supino_reto", "desenvolvimento_halteres", "crucifixo_halteres", "triceps_corda", "flexao_braco"],
      pull: ["puxada_pulley", "remada_curvada", "remada_unilateral", "rosca_direta"],
      legs: ["agachamento_livre", "terra_romeno", "leg_press", "cadeira_extensora", "panturrilha_pe"]
    };

    // State Variables
    let currentWorkout = 'push';
    let currentModalExerciseId = null;
    ...
    // Handle Image Error Gracefully with custom SVG
    function handleImageError(imgElement) {
      const exerciseId = imgElement.getAttribute('data-ex-id') || 'Treino';
      const ex = exercisesDB[exerciseId];
      const title = ex ? ex.name : 'Exercício';
      imgElement.src = generateFallbackSVG(title, '#38bdf8');
    }

    // Tab Switching Logic
    function switchTab(tab) {
      const tabs = ['workouts', 'nutrition', 'gallery', 'progress'];
      tabs.forEach(t => {
        const el = document.getElementById(`tab-${t}`);
        const btn = document.getElementById(`nav-${t}`);
        if (t === tab) {
          el.classList.remove('hidden');
          btn.className = "flex flex-col items-center gap-1 active-nav-btn transition";
        } else {
    ...
    function setWorkout(key) {
      ...
      btnPush.className = "flex flex-col items-center justify-center py-2.5 px-2 rounded-xl border border-slate-800 bg-slate-900 text-slate-400 font-semibold text-xs transition active:scale-95";
      btnPull.className = "flex flex-col items-center justify-center py-2.5 px-2 rounded-xl border border-slate-800 bg-slate-900 text-slate-400 font-semibold text-xs transition active:scale-95";
      btnLegs.className = "flex flex-col items-center justify-center py-2.5 px-2 rounded-xl border border-slate-800 bg-slate-900 text-slate-400 font-semibold text-xs transition active:scale-95";

      if (key === 'push') {
        btnPush.className = "flex flex-col items-center justify-center py-2.5 px-2 rounded-xl border border-rose-500/50 bg-rose-950/40 text-rose-400 font-bold text-xs transition active:scale-95 shadow-lg shadow-rose-950/20";
        document.getElementById('workout-label').innerText = "Treino A — Push (Peito, Ombro e Tríceps)";
      } else if (key === 'pull') {
        btnPull.className = "flex flex-col items-center justify-center py-2.5 px-2 rounded-xl border border-sky-500/50 bg-sky-950/40 text-sky-400 font-bold text-xs transition active:scale-95 shadow-lg shadow-sky-950/20";
        document.getElementById('workout-label').innerText = "Treino B — Pull (Costas e Bíceps)";
      } else if (key === 'legs') {
        btnLegs.className = "flex flex-col items-center justify-center py-2.5 px-2 rounded-xl border border-emerald-500/50 bg-emerald-950/40 text-emerald-400 font-bold text-xs transition active:scale-95 shadow-lg shadow-emerald-950/20";
        document.getElementById('workout-label').innerText = "Treino C — Legs (Pernas Completas & Panturrilha)";
      }

      renderWorkoutExercises();
    }

    function getExerciseImage(id) {
      const customImg = localStorage.getItem(`custom_img_${id}`);
      if (customImg) return customImg;
      return exercisesDB[id] ? exercisesDB[id].img : generateFallbackSVG('FitPulse');
    }

    function renderWorkoutExercises() {
      const container = document.getElementById('exercises-container');
      const ids = workoutRoutines[currentWorkout] || [];
      document.getElementById('workout-stats-badge').innerText = `${ids.length} exercícios`;
      ...
      ids.forEach((id, index) => {
        ...
        const imgSrc = getExerciseImage(id);
        const savedKg = localStorage.getItem(`weight_${id}`) || '';
        const savedReps = localStorage.getItem(`reps_${id}`) || '';
        const isDone = localStorage.getItem(`done_${id}`) === 'true';

        html += `
          <div class="bg-slate-900 border ${isDone ? 'border-emerald-500/50 bg-emerald-950/10' : 'border-slate-800 hover:border-slate-700'} rounded-2xl p-3.5 transition-all shadow-md">
            
            <div class="flex items-center gap-3">
              <!-- Exercise Image Thumbnail -->
              <div onclick="openExerciseModal('${id}')" class="w-20 h-20 bg-slate-950 rounded-xl overflow-hidden border border-slate-700/80 flex-shrink-0 cursor-pointer relative group flex items-center justify-center p-0.5">
                <img src="${imgSrc}" alt="${ex.name}" data-ex-id="${id}" class="w-full h-full object-cover rounded-lg group-hover:scale-105 transition duration-300" onerror="handleImageError(this)">
                <div class="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 flex items-center justify-center transition rounded-lg">
                  <i class="fa-solid fa-up-right-and-down-left-from-center text-cyan-400 text-xs"></i>
                </div>
              </div>
              ...
              <div class="flex-1 min-w-0">
                <div class="flex items-center gap-2 mb-0.5">
                  <span class="text-[9px] font-extrabold px-1.5 py-0.5 rounded bg-slate-800 text-slate-300">#${index + 1}</span>
                  <span class="text-[10px] text-slate-400 font-medium truncate">${ex.category.split('•')[0]}</span>
                </div>
                <h4 onclick="openExerciseModal('${id}')" class="text-sm font-bold text-white tracking-tight leading-snug cursor-pointer hover:text-cyan-400 truncate">
                  ${ex.name}
                </h4>
                <div class="flex flex-wrap items-center gap-2.5 text-[11px] text-slate-300 mt-1 font-medium">
                  <span class="text-amber-400"><i class="fa-solid fa-layer-group text-[9px] mr-1"></i>${ex.series}</span>
                  <span><i class="fa-solid fa-repeat text-[9px] mr-1 text-slate-400"></i>${ex.reps}</span>
                  <span class="text-cyan-400"><i class="fa-solid fa-clock text-[9px] mr-1"></i>${ex.rest}</span>
                </div>
              </div>

              <!-- View Details Button -->
              <button onclick="openExerciseModal('${id}')" class="px-2.5 py-2 rounded-xl bg-slate-800 hover:bg-slate-700 text-cyan-400 flex flex-col items-center justify-center transition active:scale-95 flex-shrink-0">
                <i class="fa-solid fa-eye text-xs"></i>
                <span class="text-[9px] font-bold text-slate-300 mt-0.5">Guia</span>
              </button>
            </div>
            ...
            <div class="mt-3 pt-2.5 border-t border-slate-800/90 flex items-center justify-between gap-2">
              <div class="flex items-center gap-1.5 flex-1">
                <span class="text-[11px] font-medium text-slate-400">Carga:</span>
                <input type="text" placeholder="ex: 30kg" value="${savedKg}" onchange="saveExerciseLog('${id}', 'weight', this.value)" class="w-full max-w-[85px] bg-slate-950 border border-slate-800 rounded-lg px-2 py-1 text-xs text-white font-semibold focus:outline-none focus:border-cyan-500">
              </div>
              <div class="flex items-center gap-1.5 flex-1">
                <span class="text-[11px] font-medium text-slate-400">Reps:</span>
                <input type="text" placeholder="ex: 10-10-8" value="${savedReps}" onchange="saveExerciseLog('${id}', 'reps', this.value)" class="w-full max-w-[90px] bg-slate-950 border border-slate-800 rounded-lg px-2 py-1 text-xs text-white font-semibold focus:outline-none focus:border-cyan-500">
              </div>
              <button onclick="toggleExerciseDone('${id}')" class="px-3 py-1.5 rounded-lg text-xs font-bold transition flex items-center gap-1.5 ${isDone ? 'bg-emerald-600 text-white' : 'bg-slate-800 hover:bg-slate-700 text-slate-300'}">
                <i class="fa-solid ${isDone ? 'fa-check-double' : 'fa-check'} text-[10px]"></i>
                <span>${isDone ? 'Feito' : 'Check'}</span>
              </button>
            </div>
    ...
    function handleUserPhotoUpload(event) {
      ...
      reader.onload = function(e) {
        ...
        try {
          ...
          // If storage quota exceeded, alert subtly without native alert
      ...
      keys.forEach(key => {
        ...
        html += `
          <div onclick="openExerciseModal('${key}')" class="bg-slate-950 border border-slate-800 rounded-xl overflow-hidden cursor-pointer hover:border-cyan-500 transition group p-2">
            <div class="w-full h-28 bg-slate-900 rounded-lg overflow-hidden relative mb-2">
              <img src="${imgSrc}" alt="${ex.name}" data-ex-id="${key}" class="w-full h-full object-cover group-hover:scale-105 transition duration-300" onerror="handleImageError(this)">
